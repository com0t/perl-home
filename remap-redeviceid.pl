...
eval {
	...
	my $skipids;
	my $vmwareidQ = $db->prepare("select distinct deviceid from riscvmwarematrix");
	$vmwareidQ->execute();
	...
	my $tables = $db->selectall_hashref("select table_name
	...
	foreach my $tbl (sort keys %{$tables}) {
		dbg("====> $tbl");
		my $update = $db->prepare("update $tbl set deviceid = ? where deviceid = ?");
		my $deviceids = $db->selectall_hashref("select distinct deviceid from $tbl",'deviceid');
	...
	eval {
		$db->do("update credentials set uniqueid = concat(deviceid,'-',technology)");
	}; if ($@) {
	...
	my $critintupdate = $db->prepare('update critical_interfaces set connecteddeviceid = ? where (deviceid = ? and connecteddeviceid = ?)');
	my $critint = $db->prepare('select deviceid,connecteddeviceid from critical_interfaces where connecteddeviceid is not null');
	$critint->execute();
	while (my $ci = $critint->fetchrow_hashref()) {
		...
		$post =~ s/^$old/$new/;
		eval {
			$critintupdate->execute($post,$ci->{'deviceid'},$pre);
		}; if ($@) {
	...
	$riscdevice->execute();
	my $rdupdate = $db->prepare('update riscdevice set deviceid = ?, macaddr = ? where (deviceid = ? and macaddr = ?)');
	while (my $rd = $riscdevice->fetchrow_hashref()) {
		...
		eval {
			if ($rd->{'deviceid'} eq $rd->{'macaddr'}) {
				$rdupdate->execute($post,$post,$rd->{'deviceid'},$rd->{'deviceid'});
			} else {
				$rdupdate->execute($post,$rd->{'macaddr'},$rd->{'deviceid'},$rd->{'macaddr'});
			}
		}; if ($@) {
	...
	## riscvmwarematrix's deviceid is the small deviceid
	## the vcenterid is also likely a large deviceid, so we need to update that as well
	## the big deviceid that correlates to a windows/gensrv is the windowsosid, and needs to be updated
	## we processed the vcenterid as the deviceid in the other vmware tables during the main table loop
	dbg('processing riscvmwarematrix');
	dbg('====> vcenterid');
	my $vcenterupdate = $db->prepare('update riscvmwarematrix set vcenterid = ? where vcenterid = ?');
	my $vcenters = $db->selectall_hashref('select distinct vcenterid from riscvmwarematrix','vcenterid');
	foreach my $vc (keys %{$vcenters}) {
		next if ($skipids->{$vc});
		my $pre = $vc;
		my $post = $pre;
		$post =~ s/^$old/$new/;
		eval {
			$vcenterupdate->execute($post,$pre);
		}; if ($@) {
			print STDERR "EXCEPTION: riscvmwarematrix/windowsosid: $@";
		}
	}
	dbg('====> windowsosid');
	my $winosupdate = $db->prepare('update riscvmwarematrix set windowsosid = ? where windowsosid = ?');
	my $winosid = $db->selectall_hashref('select distinct windowsosid from riscvmwarematrix','windowsosid');
	foreach my $wos (keys %{$winosid}) {
		my $pre = $wos;
		my $post = $pre;
		$post =~ s/^$old/$new/;
		eval {
			$winosupdate->execute($post,$pre);
		}; if ($@) {
			print STDERR "EXCEPTION: riscvmwarematrix/windowsosid: $@";
		}
	}
	...
	dbg('db_inventory');
	my $dbhostupdate = $db->prepare('update db_inventory set hostdevice = ? where (dboid = ? and hostdevice = ?)');
	my $dbinvQ = $db->prepare('select dboid,hostdevice from db_inventory');
	$dbinvQ->execute();
	while (my $hd = $dbinvQ->fetchrow_hashref()) {
		next if (($hd->{'dboid'} eq $hd->{'hostdevice'}) or ($skipids->{$hd->{'hostdevice'}}));
		my $pre = $hd->{'hostdevice'};
		my $post = $pre;
		$post =~ s/^$old/$new/;
		eval {
			$dbhostupdate->execute($post,$hd->{'dboid'},$hd->{'hostdevice'});
		}; if ($@) {
