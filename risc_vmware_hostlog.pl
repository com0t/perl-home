...
my $insertlog = $mysql->prepare_cached("INSERT IGNORE into vmware_hostlogs (deviceid,scantime,logdatetime,logfile,hostname,logentry,logid) values (?,?,?,?,?,?,?)");
...
my $scantime = time();
my $risclog = Opts::get_option('log');
...
sub getLogData {
   ...
   foreach (@$host_view) {
      ...
      eval {
      	 my $logfileDescriptions = $diagmgr_view->QueryDescriptions(host=>$_);
      	 foreach my $log (@$logfileDescriptions) {
      	 	my $logfileName = $log->key;
      	 	print "Getting the Log file: $logfileName\n";
         if ($instance_type eq 'VirtualCenter') {
            $logData = $diagmgr_view->BrowseDiagnosticLog(key => $logfileName,
                                                          host => $_,
                                                          start => "999999999");
            $lineEnd = $logData->lineEnd;
            $start = $lineEnd - $lastNLines;
            $logData = readLogFile($diagmgr_view, $logfileName, $start);
         }
         else {
            $logData = $diagmgr_view->BrowseDiagnosticLog(key => $logfileName,
                                                          start => "999999999");
...
sub readLogFile {
   ...
   if ($logData->lineStart != 0) {
        ...
        ##Added by JL
        $line =~ /.^*(....-..-..\s+..:..:..\....)\s+(.*)$/;
        my $logid=md5_hex($deviceid,$1,$2);
        $insertlog->execute($deviceid,$scantime,$1,$risclog,$logKey,$2,$logid);
         }
    }
   return $logData;
}
